...
describe('remoteInsert() Test:', () => {
  ...
  it('1. remoteInsert : A랑 B 사이에 문자 하나가 삽입되는 경우', () => {
    const [insertedIndex, insertedChars] = crdt.remoteInsert([new Char(A.id, B.id, 'siteId', 'D')]);
    expect(insertedIndex).toEqual(1);
    expect(insertedChars).toEqual('D');
    expect(crdt.toString()).toEqual('ADBC');
  });

  it('2. remoteInsert : 맨 앞에 문자 하나를 입력받는 경우', () => {
    const [insertedIndex, insertedChars] = crdt.remoteInsert([new Char(crdt.head.id, A.id, 'siteId', 'D')]);
  ...
  it('3. remoteInsert : 맨 뒤에 문자 하나를 입력받는 경우', () => {
    const [insertedIndex, insertedChars] = crdt.remoteInsert([new Char(C.id, crdt.tail.id, 'siteId', 'D')]);
    expect(insertedIndex).toEqual(3);
    expect(insertedChars).toEqual('D');
    expect(crdt.toString()).toEqual('ABCD');
  });

  it('4. remoteInsert : 여러 개를 맨 뒤에 입력하는 경우', () => {
    const [insertedIndex, insertedChars] = crdt.remoteInsert([
      new Char(C.id, 'E_id', 'siteId', 'D', 'D_id'),
      new Char('D_id', 'F_id', 'siteId', 'E', 'E_id'),
      new Char('E_id', crdt.tail.id, 'siteId', 'F', 'F_id')
    ]);
    expect(insertedIndex).toEqual(3);
    expect(insertedChars).toEqual('DEF');
  ...
  it('7. remoteInsert : localInsertRange 로 글자 사이에 입력', () => {
    const [insertedIndex, insertedChars] = crdt.remoteInsert(crdt.localInsertRange(1, 'DEF'));
    expect(insertedIndex).toEqual(1);
    expect(insertedChars).toEqual('DEF');
    expect(crdt.toString()).toEqual('ADEFBC');
  });
...
describe('remoteDelete() Test:', () => {
  ...
  it('1. remoteDelete 테스트 : 맨 앞에 있는 글자 하나만 삭제  ', () => {
    expect(crdt.remoteDelete([A])).toEqual([0, 1]);
    expect(crdt.toString()).toEqual('BC');
  });

  it('2. remoteDelete 테스트 : 다 지우기', () => {
    expect(crdt.remoteDelete([A, B, C])).toEqual([0, 3]);
    expect(crdt.toString()).toEqual('');
  });
  
  it('3. remoteDelete 테스트 : 맨 뒤에 있는 글자 하나만 삭제', () => {
    expect(crdt.remoteDelete([C])).toEqual([2, 3]);
