...
const localDeleteBenchmark = (type: string) => {
  ...
  for (let i = 1; i <= caseLength; i++) {
    ...
    template[operationTime.toString()] = calcTimeDiff(crdt.localDelete.bind(crdt), [0, 1], operationTime, type);
  }
  
  console.table(template);
}

export { localDeleteBenchmark };