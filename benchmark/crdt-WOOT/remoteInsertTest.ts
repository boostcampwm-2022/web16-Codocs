  ...
  for (let i = 1; i <= caseLength; i++) {
    ...
    crdt.syncDocument(crdtOthers.charMap);
      ...
      for(let k = 0; k < operationTime; k++) {
        const newChar = crdtOthers.localInsert(0, 'a')
        operationHistory.push({...newChar});
      }
    } else if (type === "WORST") {
        for(let k = 0; k < operationTime; k++) {
          const newChar = crdtOthers.localInsert(Math.floor(operationTime/2), 'a')
          operationHistory.push({...newChar});
        }
        
    } else if (type === "RANDOM") {
        for(let k = 0; k < operationTime; k++) {
          const newChar = crdtOthers.localInsert(Math.floor(Math.random() * (operationTime + k + 1)), 'a')
          operationHistory.push({...newChar});
        }
    }
